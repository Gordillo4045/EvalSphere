...
const FormularioPrueba = () => {
  ...
  const resetFormData = (questionsData: Question[]) => {
  ...
  const checkAllDefaultValues = (data: FormData): boolean => {
    return Object.values(data).every(value => value === 1);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (checkAllDefaultValues(formData)) {
  ...
  const submitEvaluation = async (dataToSend: FormData) => {
    try {
      const dataToSubmit = {
        ...dataToSend,
        comments,
        ...
        <ModalContent>
          {(onClose) => (
            <>
              ...
              <ModalBody>
                <p>
                  Todas las respuestas tienen el valor mínimo "Muy bajo".

                </p>
                <p className='text-sm'>
                  ¿Estás seguro de que deseas enviar la evaluación con estos valores?
                </p>
              </ModalBody>
              <ModalFooter>
                <Button
                ...
                <Button
                  ...
                  onPress={() => {
                    if (pendingSubmission) {
                      submitEvaluation(pendingSubmission);
                      onClose();
                    }
                  }}
                >
